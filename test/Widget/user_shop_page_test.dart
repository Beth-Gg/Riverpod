...
void main() {
  testWidgets('UserShopPage displays shop items', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          authProvider.overrideWith(Provider((ref) => mockAuthProvider)
              as Create<AuthState, ProviderRef<AuthState>>),
          shopsProvider.overrideWith(Provider((ref) => mockShops) as Create<
              ShopsNotifier,
              StateNotifierProviderRef<ShopsNotifier, List<Shop>>>),
        ],
        child: MaterialApp(
          home: UserShopPage(),
        ),
    ...
    print('Widget tree built, starting assertions');

    // Verify that the app bar title is correct
    expect(find.text('Shemeta Shopping'), findsOneWidget);

    print('App bar title verified');
